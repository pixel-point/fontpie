...
program
  ...
  .option('-f, --fallback <serif|sans-serif>', 'Fallback font family: serif or sans-serif', 'serif')
  .option('-s, --style <normal|italic>', 'font-style property', 'normal')
  .option('-w, --weight <number>', 'font-weight property', '400')
  .option('-n, --name <string>', 'Font name what will be used for font-family property. Font filename by default')
  .action((file, option) => {
    // ToDo: try...catch
    const font = fontkit.openSync(file)

    const { ascentOverride, descentOverride, lineGapOverride, fallbackFont, sizeAdjust } = calculateFallbackFontValues(font, option.fallback)
...
html {
  font-family: '${path.parse(file).name}', '${path.parse(file).name} Fallback';
}
